...
function drawHeaderPhoenix(time = 0) {
  ...
  const pixelsWide = 200, pixelsHigh = Math.round(pixelsWide * image.height / image.width);
  ...
  const scaleX = pixelsWide / image.width, scaleY = pixelsHigh / image.height;
  const flapAngle = Math.sin(time / 150) * .085;
  const drawPart = (points, anchorX, anchorY, angle = 0) => {
    sourceContext.save();
    sourceContext.scale(scaleX, scaleY);
    sourceContext.beginPath();
    points.forEach(([x, y], index) => index ? sourceContext.lineTo(x, y) : sourceContext.moveTo(x, y));
    sourceContext.closePath();
    sourceContext.clip();
    sourceContext.translate(anchorX, anchorY);
    sourceContext.rotate(angle);
    sourceContext.translate(-anchorX, -anchorY);
    sourceContext.drawImage(image, 0, 0);
    sourceContext.restore();
  };
  // Broad overlapping masks meet under the body at the true shoulder joints.
  drawPart([[0, 0], [410, 0], [470, 420], [525, 545], [490, 690], [120, 720], [0, 570]], 485, 555, flapAngle);
  drawPart([[750, 0], [1122, 0], [1122, 610], [985, 720], [645, 690], [625, 545], [700, 420]], 645, 555, -flapAngle);
  // The complete head, crest, torso, and tail form one unmoving layer over both joints.
  drawPart([[390, 0], [805, 0], [790, 475], [900, 700], [1122, 1398], [0, 1398], [245, 700], [350, 475]], 0, 0);
  const warmPalette = sourceContext.createLinearGradient(0, 0, pixelsWide, pixelsHigh);
  warmPalette.addColorStop(0, "#ffb20f"); warmPalette.addColorStop(.48, "#ff4b3f"); warmPalette.addColorStop(.76, "#f20e8d"); warmPalette.addColorStop(1, "#7d35dd");
  sourceContext.globalCompositeOperation = "source-atop";
  sourceContext.globalAlpha = .55;
  sourceContext.fillStyle = warmPalette;
  sourceContext.fillRect(0, 0, pixelsWide, pixelsHigh);
...
function animateHeaderPhoenix(time) {
  drawHeaderPhoenix(time);
  headerAnimationFrame = window.requestAnimationFrame(animateHeaderPhoenix);
}
...
image.src = "phoenix-cleaned.png"; image.onload = () => { draw(); if (headerAnimationFrame) window.cancelAnimationFrame(headerAnimationFrame); headerAnimationFrame = window.requestAnimationFrame(animateHeaderPhoenix); }; window.addEventListener("resize", () => { draw(); drawHeaderPhoenix(performance.now()); }); refresh().catch(() => { document.querySelector("#loading-message").textContent = "Live RSVP progress is temporarily unavailable."; }); window.setInterval(() => refresh().catch(() => {}), 60_000);
