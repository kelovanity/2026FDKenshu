...
function drawHeaderPhoenix(time = 0) {
  ...
  const flap = .83 + Math.sin(time / 105) * .17;
  const scaleX = pixelsWide / image.width, scaleY = pixelsHigh / image.height;
  const drawWing = (sx, sy, sw, sh, anchorX, anchorY) => {
    sourceContext.save();
    sourceContext.translate(anchorX * scaleX, anchorY * scaleY);
    sourceContext.scale(1, flap);
    sourceContext.translate(-anchorX * scaleX, -anchorY * scaleY);
    sourceContext.drawImage(image, sx, sy, sw, sh, sx * scaleX, sy * scaleY, sw * scaleX, sh * scaleY);
    sourceContext.restore();
  };
  // Wings are drawn independently around their roots, then the body is placed over them.
  drawWing(0, 0, 535, 710, 490, 520);
  drawWing(590, 0, 532, 710, 635, 520);
  sourceContext.drawImage(image, 300, 105, 540, 1293, 300 * scaleX, 105 * scaleY, 540 * scaleX, 1293 * scaleY);
  ...
  sourceContext.globalCompositeOperation = "color";
  sourceContext.globalAlpha = .68;
